...
const formatearFechaArgentina = (fechaString) => {
  ...
  try {
    ...
    // CORREGIDO: Ajustar para zona horaria de Argentina
...
// CORREGIDO: Función de adaptación mejorada para el frontend con métodos de pago y fechas
export const adaptVentaToFrontend = (venta) => {
  ...
  try {
    return {
      ...
      fecha: formatearFechaArgentina(venta.fecha), // CORREGIDO: Usar formatearFechaArgentina
      ...
      // CORREGIDO: Array de pagos individuales con fechas formateadas
      pagos: Array.isArray(venta.pagos)
        ? venta.pagos.map((pago) => ({
            id: Number(pago.id) || 0,
            monto: Number(pago.monto) || 0,
            fecha: formatearFechaArgentina(pago.fecha) || "", // CORREGIDO: Usar formatearFechaArgentina
